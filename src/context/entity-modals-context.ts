...
export const EntityModalsContext = createContext<EntityModalsContextValue | null>(null);

export function useEntityModals() {
  const ctx = useContext(EntityModalsContext);
  if (!ctx) throw new Error("useEntityModals must be used within EntityModalsProvider");
  return ctx;
}
