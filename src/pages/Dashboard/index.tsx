    ...
    ? [
        {
          ...
          value: dashboard.data.total_credit_given,
          icon: Coins,
          tint: "primary",
        },
        {
          label: "Total recovered",
          value: dashboard.data.total_recovered,
          icon: Wallet,
          tint: "success",
        },
        {
          label: "Total pending",
          value: dashboard.data.total_pending,
          icon: Clock,
          tint: "warning",
        },
        {
          label: "Today's udharo",
          value: dashboard.data.todays_udharo,
          icon: CalendarDays,
          tint: "accent",
        },
      ]
    ...
    <div className="space-y-8">
      ...
      {dashboard.isError && (
        <Alert
          ...
          description={(dashboard.error as Error).message}
        />
      )}
          ...
          <ol className="space-y-2">
            {dashboard.data?.top_5_debtors?.map((c, i) => (
              <li key={c.id}>
                <Link
                  to={`/customers/${c.id}`}
                  className="flex items-center gap-4 p-3 rounded-2xl hover:bg-muted/60 transition"
                >
            ...
            {dashboard.data &&
              (dashboard.data.top_5_debtors ?? []).length === 0 && (
        ...
        <Panel>
          ...
          {udharoEntries.isError && (
            <Alert
              ...
              description={(udharoEntries.error as Error).message}
            />
          )}
